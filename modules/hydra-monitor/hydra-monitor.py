#!/usr/bin/env python
# Initial code use from:
#   https://github.com/NixOS/nixos-org-configurations/blob/master/delft/prometheus/hydra-queue-runner-reexporter.py
...
def debug_remaining_state(edict):
    # log.info(edict.remaining_state())
    pass
...
class HydraScrapeImporter:
    ...
    def collect(self):
        ...
        yield self.trivial_gauge(
            "notification_time_avg_ms",
            "Average notification time in ms",
            self.destructive_read_default("nrNotificationTimeAvgMs", 0)
        )
...
if __name__ == '__main__':
    ...
    start_http_server(@bindingPort@, addr="@bindingAddress@", registry=registry)
